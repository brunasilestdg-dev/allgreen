...
Este arquivo é o guia único para qualquer pessoa ou assistente que trabalhe neste repositório. Diferentes contribuidores se revezam no projeto — **leia tudo antes de alterar o código.**
...
- Frontend: React 19 + Vite. `src/App.jsx` ainda concentra o app geral; o acesso (login, cadastro, convite, primeiro acesso) mora em `src/features/auth/` e a checagem da sessão em `src/session/`; a vertical To Do Green em `src/features/logistics/`; `src/styles.css` os estilos (ver [Arquitetura](#arquitetura-do-frontend-quebra-do-monólito-em-andamento))
- Backend: Cloudflare Worker. A entrada publicada é `worker-entry.js`; o app geral (login, chat de IA multi-provedor, mídia, sincronização, colaboração) é composto pelo `worker.js` a partir da tabela de rotas `worker/http/routes.js` e dos serviços em `worker/services/` (ver [Arquitetura do backend](#arquitetura-do-backend))
...
**Lint** (`eslint.config.js`, flat config): roda no `verify`, no deploy local completo e no CI. O Cloudflare Workers Builds também deve executar o gate mínimo (`npm ci`, `npm run verify`, `npm run build`) antes de publicar; GitHub Actions vermelho por falta de minutos/runner não bloqueia, mas `verify`, `build`, `test:e2e:critical`, Cloudflare Builds ou deploy manual vermelho bloqueia. O lint trava só em ERROS: as regras de `eslint:recommended` — inclusive `no-undef`, que pega o nome que ficou sem import depois de mover código de arquivo — e `react-hooks/rules-of-hooks` (guarda contra a classe de bug de "hooks depois de return condicional" que já mordeu aqui). O resto é AVISO (backlog para reduzir aos poucos, ~220): `no-unused-vars`, `no-empty`, `react-hooks/exhaustive-deps`, regras novas do React Compiler (`set-state-in-effect` etc.) e `jsx-a11y` (acessibilidade). Ao mexer no código, não precisa zerar os avisos, mas **não introduza erros** (o CI barra).
...
`GEMINI_API_KEY`, `XAI_API_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_API_KEY`, `BREVO_API_KEY`, `MAIL_SENDER`, `MAIL_SENDER_NAME`. Os provedores gratuitos adicionais são ativados quando seus segredos `GROQ_API_KEY`, `SAMBANOVA_API_KEY`, `CEREBRAS_API_KEY`, `MISTRAL_API_KEY`, `OPENROUTER_API_KEY` e `HF_TOKEN` forem cadastrados (o GitHub Models foi aposentado em 30/07/2026 e saiu da cascata). Lista completa de variáveis em `docs/SECRETS.md`.

**Pendente de cadastro pela responsável pela conta** (a sessão que implementou não tem acesso ao `wrangler login`/cofre de produção): `VAPID_PUBLIC_KEY` e `VAPID_PRIVATE_KEY` — sem eles, `pushEnabled(env)` fica `false` e o app funciona normalmente, só sem notificações do navegador. Gerar um par novo com `crypto.subtle.generateKey({name:"ECDSA", namedCurve:"P-256"}, true, ["sign","verify"])`, exportar a chave pública como `raw` (base64url) e a privada como `jwk.d` (o par é consumido por `pushEnabled` e pelo envio em `worker/mensageria/envio.js`, com a biblioteca `@block65/webcrypto-web-push`). Opcional: `VAPID_SUBJECT` (um `mailto:` ou URL identificando o operador) — se ausente, usa a URL de produção como padrão.
...
9. Mudanças concluídas e validadas devem ser publicadas automaticamente, sem pedir
   uma autorização adicional à responsável pela conta. Só interromper quando houver bloqueio externo
...
`src/App.jsx` ainda é grande (~15 mil linhas). O fluxo de acesso já saiu: `src/features/auth/` tem as telas (`Login.jsx`, `AcceptInvite.jsx`, `FirstAccessPassword.jsx`…), as regras puras e testadas (`authDomain.js`: validação, destino depois do login, quando exigir a troca da senha provisória) e as chamadas à API (`authApi.js`); `src/session/useStatusDaSessao.js` confere a sessão no Worker e só um 401 encerra a sessão local. A **camada de lógica pura** (sem React/JSX) está sendo movida para `src/domain.js`: helpers fundamentais (`uid`, `today`, `contactLinks`), gamificação (`DEFAULT_LEVELS`, `computeUserPoints`, `levelForPoints`, `levelProgress`, `computeAchievements`), `computeMyWork` e `computeBusinessInsights`. **Padrão:** `App.jsx` importa de `./domain.js` para uso interno **e reexporta** o que os testes consomem via `import { ... } from "./App"` — assim testes existentes não quebram. Ao adicionar nova lógica pura/testável, colocar em `domain.js` (não em `App.jsx`) e, se algum teste importar de `./App`, adicionar à linha de reexport. Próximos passos da quebra (ainda não feitos): extrair componentes de página com `React.lazy` para code-splitting.

### Vertical To Do Green (`src/features/logistics/`)

`LogisticsVertical.jsx` é a casca: estado, dados, cabeçalho, menu e a composição das telas. Em volta dela:

- `shell/` — JS puro, sem React: catálogo de módulos (`catalogoDeModulos.js`), menu e permissões por tela (`navegacao.js`, com `PRIMARY_NAVIGATION`), rotas (`rotas.js`), resposta de acesso (`acesso.js`) e o mapeamento dos registros da API para as telas (`dadosDaVertical.js`).
- `journeys/` — os painéis de jornada que moravam na casca (precificação, proposta, ESG, acessos) e os componentes compartilhados.
- `areas/` — um componente por área do menu, que só despacha a tela da área. **Sem hooks nem estado**: `areas/despachoDasAreas.test.jsx` os chama como função e trava que cada tela sai de exatamente um grupo e que cada página é importada dinamicamente uma vez só. Tela nova entra no catálogo, no menu e no grupo da área dona.

## Arquitetura do backend

- **Entrada**: `worker-entry.js` (`wrangler.jsonc → main`) atende primeiro a vertical — interceptações próprias e `worker/services/todogreen-router.js` — e entrega o resto ao `worker.js`, que só compõe `fetch` e `scheduled`.
- **Rotas do app geral**: tabela única e ordenada em `worker/http/routes.js` (`ROTAS_PUBLICAS`, `ROTAS_AUTENTICADAS`); `worker/http/router.js` despacha, valida a sessão num ponto só e envolve toda rota no mesmo guarda de erro (JSON 500 em português, com rótulo no log). `/api/*` sem rota responde JSON 404. **Rota nova de dados entra em `ROTAS_AUTENTICADAS` e só lá** — é a entrada que liga a checagem de sessão e a de banco. `test/tabela-de-rotas.worker.test.js` trava a tabela (401 sem sessão em toda rota autenticada, listas esperadas, páginas públicas presentes no `run_worker_first` do `wrangler.jsonc`) e `test/roteador.worker.test.js` caracteriza status, corpo e rótulos de cada rota.
- **Serviços**: um arquivo por assunto em `worker/services/`, com o contrato no topo (o que recebe, o que devolve, quem chama, a regra de autorização). Utilitários de autorização por registro em `worker/lib/` (`visibility.js`, `record-permissions.js`).
- **Registros da vertical**: `todogreen-vertical-records.js` é a fachada; o CRUD, os gates e os efeitos ficam em `worker/services/vertical-records/`, e cada coleção é um descritor em `vertical-records/colecoes/<domínio>.js` com o formato validado por `colecoes/descritor.js` e travado em `src/features/logistics/verticalRecordsContract.test.js`. Coleção nova = descritor novo no domínio certo, sem handler.
- **Portal do Cliente**: `todogreen-customer-portal.js` é a fachada (sessão, escopo, limite); as rotas ficam em `worker/services/customer-portal/rotas/`. O que sai para o cliente é lista fechada — `customerPortalViewContract.test.js` e `test/todogreen-portal-contrato.worker.test.js` reprovam campo interno no payload. A API interna de clientes mora em `todogreen-clients.js`.
- **Crons**: os padrões ficam em `worker/lib/cron.js` e precisam bater com `triggers.crons`; o disparo semanal roda só o resumo semanal, os demais jobs rodam no horário (`test/cron.worker.test.js`).
...
- **IA**: cascata dinâmica em `worker/services/ai.js` (`providerChain`): Google Gemini/Gemma, Cloudflare Workers AI, Groq, SambaNova, Cerebras, Mistral, OpenRouter, Hugging Face e IA auto-hospedada (Ollama/vLLM, com URL + modelo). **Rota sensível (LGPD)**: pedido com CPF, cartão, senha/chave, conta bancária ou termo clínico (`pedidoSensivel`) usa só quem não treina com o conteúdo — ver seção "Auditoria de 24/09/2026". Só entram provedores configurados; xAI exige confirmação paga; ao final há contingência local. Opcional: **AI Gateway** da Cloudflare na frente da cascata (`AI_GATEWAY_ID`) e **Prompt Guard** no conteúdo externo — ver "Segunda leva do catálogo". 46 funcionários especialistas + Diretor orquestrador + funcionários dinâmicos criados pelo usuário
...
- **Memória da IA + busca por significado** (`src/features/knowledge/memoryDomain.js`, `searchDomain.js` + `KnowledgeCenter.jsx`, lazy) — coleções `memories` e `glossary`; blocos 3 e 4 da segunda lista. **Memória**: escopo (`pessoal|empresa|projeto|cliente|especialista`) com `scopeRef`, e `relevantMemories` **nunca vaza** memória de projeto/cliente/especialista fora do contexto correspondente nem devolve memória `approved: false`. `detectSensitive` (CPF, CNPJ, cartão, senha, conta, saúde) faz a memória **nascer pendente**, e `makeMemory` marca `approved: false` — aprovação é da responsável pela conta. `findConflicts` separa **contradição de repetição** comparando negação (`não/nunca/sem/exceto`) sobre pares de alta similaridade Jaccard, e só compara dentro do mesmo escopo+scopeRef. `isStale` avisa memória vencida mas **nada é apagado sozinho**. `suggestMemories` só extrai frases com gatilho declarativo e nunca inventa texto. **Busca**: `stem` é um radical simples de pt-BR (plural, `-mento`, `-ção`, `-ais`) que faz "clientes" achar "cliente"; `expandWithSynonyms` usa o glossário da empresa e é o que mais aproxima de busca semântica **sem vetor nenhum**; `scoreDocuments` é BM25 com `titleBoost` de 3. `buildIndex` **respeita visibilidade**: item `privado` de outro `ownerId` não entra no índice — a busca não pode ser um vazamento. `buildAnswerPrompt` obriga citação `[n]` e manda dizer "Não encontrei essa informação no seu workspace." em vez de completar com conhecimento geral. **Significado (desde 24/09/2026)**: o BM25 continua sendo a base e responde na hora. Por cima dele, os vetores do `bge-m3` (`/api/busca/vetores`, cache no D1 e no IndexedDB) somam por RRF o que fala do mesmo assunto sem a palavra buscada; esses itens aparecem marcados como "pelo significado". Sem IA no servidor, a tela fica só com o BM25 — ver "Segunda leva do catálogo". Testado em `src/knowledge-domain.test.js`, `src/knowledge-ui.test.jsx`, `src/semantic-search.test.js` e `test/busca-semantica.worker.test.js`.
...
- **Colaboração em equipe** (ver `worker/services/collab.js` e `src/App.jsx`, componente `Collaborators`/`Team`): convite real por e-mail com ativação (`/api/collab/invite*`, `/api/collab/join`, migração `0010_team_invites.sql`); três papéis (`admin`/`gestor`/`colaborador`, `VALID_ROLES`) — todos os papéis acessam TODAS as ferramentas, o controle é sobre os DADOS, nunca sobre o acesso à tela
  ...
  - **Falha no e-mail do convite nunca é silenciosa**: `invite`/`resend` em `worker/services/collab.js` devolvem `emailError` junto com `emailSent: false` — o motivo vem de `emailFailureReason` (`worker/mensageria/envio.js`), que traduz a recusa do Brevo (IP não autorizado, remetente não validado, chave inválida, cota, conta não liberada) ou avisa que faltam `BREVO_API_KEY`/`MAIL_SENDER`. A tela de Equipe mostra esse motivo; o link do convite continua valendo em qualquer caso.
  - **Contingência ao e-mail: acesso com senha provisória** (migração `0143`, `users.must_change_password`): na tela de Equipe, "Criar acesso com senha provisória" chama `/api/collab/create-access`, que cria a conta + `memberships` já ativa e devolve a senha provisória UMA vez (`generateTempPassword`, 12 caracteres sem ambíguos) para o admin repassar por outro canal. **Só para e-mail sem conta** (409 se já existir — definir senha de conta existente daria ao admin acesso aos outros espaços da pessoa). `/api/collab/reset-access` gera outra só enquanto a pessoa não fez o primeiro acesso; depois disso é "Esqueci minha senha". Login e `/api/auth/session` devolvem `user.mustChangePassword`; o `App` mostra `FirstAccessPassword` (`src/features/auth/`; a regra é `exigeTrocaDeSenha` em `authDomain.js`) antes de qualquer rota autenticada, e `POST /api/auth/password` (exige a senha atual) troca, desliga a marca e derruba as outras sessões. Não substitui o convite por e-mail — é o plano B.
  ...
  - **Missões e gamificação**: tarefa com `isMission: true` ganha vagas/pontos/recompensa/subtarefas/entrega-e-revisão; `computeUserPoints`/`levelForPoints`/`computeAchievements`/`levelProgress` (funções puras em `src/domain.js`) calculam pontos, nível, progresso até o próximo nível e conquistas a partir de `db.tasks` — nada fica persistido, é sempre recalculado
  ...
  - **Modal com armadilha de foco**: componente `Modal` (`src/components/Modal.jsx`) prende o Tab dentro do diálogo, foca o primeiro elemento focável ao abrir (respeitando um `autoFocus` de algum campo filho) e devolve o foco a quem abriu ao fechar — todos os modais do app usam esse componente, então a correção vale para todos de uma vez
...
- **Resumo semanal por push**: prova de valor recorrente entregue por Web Push **mesmo com o app fechado**. `computeWeeklySummary(data, start, end)` (pura: vendas/caixa/tarefas + `tasksReward` somando as recompensas das concluídas/novos contatos) existe em DUAS cópias sincronizadas: `src/domain/dasEResumoSemanal.js` (usada pelo card "Sua semana" no Dashboard do `src/App.jsx`, `#week-summary`, mostra a semana atual — com estado vazio convidativo via `hasActivity` em vez de parede de zeros) e `worker/services/weekly-summary.js` (envia o push). O envio é um **Cloudflare Cron Trigger** (`wrangler.jsonc` → `triggers.crons: ["0 12 * * 1"]`, segunda 12:00 UTC / 09:00 BRT) que dispara `export default { scheduled }` → `sendWeeklySummaries(env, now)`: itera `workspaces`, computa a semana ANTERIOR (`previousWeekBounds`), e para cada dono com atividade e assinatura ativa reserva a semana em `weekly_summary_log` (migração `0013`, `INSERT OR IGNORE` checando `meta.changes` — o Cron é "pelo menos uma vez", isso impede push duplicado) e manda o push (`link: "inicio"`); assinatura 404/410 é apagada. Semana = segunda–domingo (`weekRangeFrom`). Ao mexer na fórmula, **atualizar as duas cópias juntas** (há testes cobrindo ambas). O push server-side não escreve em `db.notifications` (evita conflito de revisão) — o card no Dashboard é a superfície in-app.
  ...
  - **Operação e parceria**: `/api/status` expõe só saúde básica, versão e se a busca web está configurada (nada de dado de conta); Configurações oferece diagnóstico sem conteúdo e canal de suporte. Planos de piloto, operação e incidentes/LGPD ficam em `docs/`. A definição de público-alvo permanece deliberadamente em aberto; não fixá-la sem decisão da responsável pela conta.
...
- **Régua de precificação versionada** (`src/features/logistics/pricingParametersDomain.js`
  ...
  XCMG da responsável pela conta. As premissas (`PREMISSAS_ATIVO_FIELDS`, editáveis pelo admin no painel
...
- **Rota do dia do motorista To Do Green** (`todogreen_routes`, migração
  ...
  em `worker/services/vertical-records/colecoes/` (`COLECOES`), sem handler novo. Regra da
...
- **O que a IA sabe da To Do Green é DADO, não prompt**: `businessContextDomain.js`
  ...
  CNH (isso se cadastra na tela, no espaço da responsável pela conta); e fato ruim documentado
...
- **Central de RFQ e RFI: status é FÓRMULA, nunca coluna**: `habilitacaoDomain.js`
  ...
  quebram: os estados são os da responsável pela conta (VENCIDO / CRÍTICO ≤30 dias / ATENÇÃO
...
- **Menu da vertical**: `PRIMARY_NAVIGATION` (`shell/navegacao.js`) é a lista
...
- **Portfólio de projetos (v144)**: lógica pura em
  ...
  (3) `pushedDays` é medido contra a data que a responsável pela conta cadastrou, não contra o
...
- **Agentes (v145)**: lógica pura em `src/features/agents/agentDomain.js` e
  ...
  `agentRuns`.
  Decisões que NÃO devem ser desfeitas sem falar com a responsável pela conta:
  ...
  da responsável pela conta em 30/07/2026; o nível avisa o que significa, vale por agente e
...
- **Conteúdo da web é dado, nunca ordem (v151)**: `webResultsToContext`
  ...
  dinheiro e podem enviar mensagem em nome da responsável pela conta. A marca é neutralizada
...
- **Gatilho da busca web (v151)**: neste app "buscar", "procurar" e "pesquisar"
  ...
  mandavam a pergunta da responsável pela conta para empresa de fora sem necessidade. Agora o
...
- **Planejamento de agente nunca busca na web (v151)**: `AgentStudio` envia
  ...
  responsável pela conta. Não remover esse parâmetro.
...
- **Modo lançamento (v153)**: `LAUNCH_MODE` em
  ...
  Decisão da responsável pela conta em 31/07/2026: 100% gratuito para todos no primeiro
...
- **Menu escolhido por quem usa (v154)**: lógica pura em
  ...
  A regra que manda em tudo, decidida pela responsável pela conta em 31/07/2026: escolher o
...
- **Layout no celular (v155)**: a queixa da responsável pela conta foi "quando abro no celular
...
- **Editor de código, Notebook e Integrações (v157)**: três telas novas em
  ...
  **Integrações não inclui envio automático** (webhook de saída) porque ele
  ...
  secreto ficaria no aparelho. Saiu do servidor depois — ver "Envio automático / webhook de saída (v158)". O que
...
- **Envio automático / webhook de saída (v158)**: `worker/services/webhooks.js`,
  ...
  **Este é o único ponto do app em que o nosso servidor busca um endereço
  ...
  A rota `/api/webhooks` PRECISA ficar em `ROTAS_AUTENTICADAS`
  (`worker/http/routes.js`); `test/tabela-de-rotas.worker.test.js` reprova se
  ela responder sem sessão. Um endereço que dispara requisições de saída sem
  login seria um amplificador aberto para qualquer um.
...
- **Jurídico canônico — UM só, ancorado no TDG**: `src/features/legal/`
  ...
  **Vocabulário canônico é o do TDG** (`legalDomain.js` da vertical) —
  ...
  aqui muda o TDG, e vice-versa; a responsável pela conta alinhou que Jurídico canônico é
...
## Auditoria de 24/09/2026 — regras que não podem voltar atrás

Resultado completo (auditoria do que já existe + catálogo de recursos gratuitos
verificados em fonte oficial) em `docs/CATALOGO_RECURSOS_GRATUITOS.md`.

- **Rota sensível da IA** (`pedidoSensivel` + `SENSITIVE_ORDER` em `ai.js`): a
  Gemini API gratuita usa o conteúdo para melhorar produtos, pode ter revisão
  humana e pede "Do not submit sensitive, confidential, or personal
  information to the Unpaid Services". Pedido com dado sensível vai só para IA
  local, Cerebras, Groq, Workers AI, SambaNova e chaves pagas do espaço; o
  streaming (que só existe pelo Gemini) devolve `fallback: true` e a tela refaz
  em `/api/ai`. A detecção olha a conversa e as memórias, nunca as instruções
  de sistema do app. **Toda nova entrada de IA precisa passar por
  `runWithFallback` ou calcular `sensitive`.**
- **GitHub Models** foi aposentado em 30/07/2026 e saiu da cascata.
- **Clima = MET Norway** (`geo-providers.js`, adaptador `horarioDaMetNorway`):
  a API gratuita do Open-Meteo é só para uso não comercial. A MET exige
  User-Agent com contato, coordenada com até 4 casas e atribuição (CC BY 4.0).
- **Roteirizador das Ferramentas**: sugestões pelo Photon (feito para
  autocompletar) e "Tempo e distância" por `/api/rotas/estimativa` (só
  Geoapify, chave no cofre). Nunca voltar a chamar o Nominatim público com
  autocompletar nem o OSRM de demonstração do navegador — as duas políticas
  proíbem. A vertical ainda cai nos públicos quando não há instância própria
  nem `GEOAPIFY_API_KEY`: configurar é pendência da titular.
- **CNPJ alfanumérico** (IN RFB 2.229/2024, emitido desde 31/07/2026):
  `normalizeDocument`/`isValidCnpj` em `erpCoreDomain.js` são a regra única
  (ASCII − 48, letra só nas 12 primeiras posições e só se fechar nos DVs).
  Não usar `replace(/\D/g, "")` em CNPJ; campo de digitação não normaliza a
  cada tecla. `gerarChaveDeAcesso` devolve `null` para emitente alfanumérico
  (a chave oficial vem do conector SEFAZ).
- **Leitor comum de arquivos** (`leituraDeArquivo.js`): imagem e PDF escaneado
  passam por OCR no aparelho (tesseract.js, até 10 páginas) e XLSX pelo
  read-excel-file; os seletores usam `DOCUMENT_ACCEPT`.
- **Cron**: o disparo semanal (`CRON_SEMANAL` em `worker/lib/cron.js`) só
  envia o resumo; os jobs horários rodam no cron de hora em hora. O rastreador
  roda só no `scheduled` do `worker.js` (rodava duas vezes por disparo).
- **QR e código de barras**: `components/QrCodeImage.jsx` (Cobrança Pix) e
  `logistics/code128Domain.js` (CODE-128C da chave no DACTE, validado lendo com
  o zxing).
- **Convite legado `?convite=`** redireciona para `/convite/:token` (a rota
  `/api/collab/join` não existe mais).

### Segunda leva do catálogo (24/09/2026)

- **Turnstile (anti-robô):** `worker/lib/turnstile.js` protege cadastro, login,
  "esqueci a senha", reenvio do código, pedido de acesso e os formulários
  públicos (`/f/`, contato do site, checkout da loja, agenda e atendimento).
  - Só liga com `TURNSTILE_SITE_KEY` **e** `TURNSTILE_SECRET_KEY`.
  - As telas do app usam a ação `entrada` (`TurnstileWidget` + `useTurnstile`
    em `components/`, ligados em `src/features/auth/Login.jsx`, e o login
    privado da vertical em `LogisticsVerticalCredentials.js`); as páginas
    públicas usam cada uma a sua.
  - Formulário público novo deve chamar `exigirTurnstile` e liberar a CSP com
    `cspComTurnstile`.
- **AI Gateway:** `worker/services/ai-gateway.js`.
  - Liga com `AI_GATEWAY_ID`. O Workers AI passa pelo binding; os provedores
    por URL só com `AI_GATEWAY_TOKEN`.
  - O gateway nunca guarda conteúdo: `collectLog: false` no binding,
    `cf-aig-collect-log-payload: false` nas URLs, e pedido sensível sem log e
    sem cache.
  - `cf-aig-no-wholesale: true` impede cobrança da Cloudflare.
  - Se o gateway falhar, a chamada vai direto ao provedor. Chamada nova de IA
    usa `fetchPeloGateway` e `rodarWorkersAi`, nunca `fetch` ou `env.AI.run`
    soltos.
- **Prompt Guard:** `worker/services/prompt-guard.js`.
  - `pareceInjecao` (heurística) roda dentro de `searchWeb` e cobre chat,
    pesquisa de empresa e radar.
  - `filtrarConteudoExterno` (heurística + Llama Prompt Guard 2 na Groq) roda
    nos resultados da web do chat.
  - `avaliarTexto` barra injeção na pergunta do portal do cliente.
  - `sanearDados` limpa o resultado de ferramenta da Semente.
  - Texto de terceiro que chega a um agente passa por uma dessas funções.
  - O modelo na Groq está em "Preview": qualquer falha dele volta para a
    heurística.
- **Busca por significado** (Memória e busca):
  - Os vetores vêm do `bge-m3` pelo `/api/busca/vetores` e ficam em cache int8
    no D1 (`busca_vetores`, migração 0145, por pessoa, sem o texto) e no
    IndexedDB do aparelho.
  - A comparação roda no aparelho e se soma ao BM25 por RRF
    (`semanticDomain.js`, `semanticSearch.js`).
  - **Vectorize descartado:** não tem simulação local, o deploy falha com
    índice inexistente, o token do Workers Builds não cria índice e o plano Free
    limita a conta a cerca de 4.900 vetores de 1.024 dimensões.
  - A saída da sessão apaga o banco `allgreen-busca` do aparelho.
...
- **Versão paralela da Codex integrada (25/09).** O commit `bab4fa0` foi publicado direto do ambiente local, fora do GitHub, e depois integrado aqui a partir do patch dos fontes.
  ...
  - Por decisão da titular, as abas de seção (Espaço, To Do e Inteligência) viraram barra segmentada, como na versão da Codex. A regra é única, em `AllGreenDesignSystem.css`; o V3 e o `LogisticsVertical.css` não estilizam mais essas abas, e a hierarquia documentada neste último foi atualizada. No Espaço, o grupo fica à direita com margem automática, não com `flex-end`, para as primeiras abas continuarem alcançáveis quando a barra rola.
...
- Google: login e envio pelo navegador (Gmail send/compose e Agenda, via token do GIS) validados em 17/07/2026, antes da separação All Green/`orianone.app`; **ler** o Gmail (`gmail.readonly`) continua sem fluxo OAuth de autorização (ver "Fórmulas nas bases"). Estado de cada integração: `docs/TODOGREEN_ERP_READINESS_MATRIX.md`
- Domínio próprio e servidor GPU de vídeo: opcionais, dependem da responsável pela conta
...
- **Tela do Planner no formato Microsoft Planner** (`src/features/logistics/pages/PlannerPage.jsx` orquestra; peças em `pages/planner/`: `PlannerSidebar` (rail "Minhas tarefas" + "Meus planos"), `PlannerBoard` + `PlannerCard` (quadro), `PlannerTable`, `PlannerTimeline` (Gantt simples), `PlannerCharts` (gráficos + radar + "Faça agora"), `PlannerTaskModal`, `plannerUi.jsx` (Avatar, Rotulo, IconePrioridade, PlanoIcone, constantes) e `planner.css` (prefixo `plr-`, carregado pela página; os formulários dos modais ainda usam `tdg-planner-form`/`tdg-planner-taskform` porque o tema escuro em `LogisticsVerticalRecovery.css` se apoia neles). Abas Quadro / Tabela / Linha do tempo / Gráficos; "Filtros" (popover multi-critério, persistido em `todogreen-planner-filtros`, com migração do antigo `todogreen-planner-status`) e "Agrupar por" (balde / progresso / responsável). **Baldes são editáveis no quadro** (adicionar coluna, renomear, mover, excluir) via `PATCH /planos/:id` com `{revision, buckets}` — helpers puros `adicionarBalde`/`renomearBalde`/`removerBalde`/`moverBalde` em `plannerDomain.js`; excluir balde move as tarefas dele para o primeiro restante (nunca apaga tarefa). **Arrastar cartão** chama `moverTarefa(id, coluna)`: por balde muda `bucketId`, por progresso muda `progress`, por responsável muda `assigneeUserId/Label` — sempre pela mesma `onUpsertCanonicalTask`, sem caminho paralelo. **Rótulos** (`labels` → `plannerLabels`) são texto livre com cor derivada por hash (`corDoRotulo`, `PALETA_ROTULOS`) — nada de campo de cor no banco; `rotulosDoPlano` alimenta sugestões e filtro. Concluídas ficam dobradas no fim da coluna (`separarConcluidas`), salvo no corte por progresso. `filtrarTarefas`/`filtrosAtivos` é o predicado único das quatro vistas; `linhaDoTempo`/`diasDaJanela` calculam em DIAS (a tela multiplica por `--plr-dia`); `graficosDoPlano` conta por status/balde/prioridade/pessoa. Testes: `plannerBoardDomain.test.js` (domínio) e `pages/PlannerPage.test.jsx` (rail, colunas, drag-and-drop, "+ Adicionar tarefa", PATCH de baldes, abas, filtros, botão "Quadro To Do" — que precisa existir MESMO sem plano, porque `LogisticsVertical.test.jsx` o procura com a rede devolvendo `{}`).
- **Compartilhar um plano é uma promessa em DUAS camadas, e a tela só pode prometer o que as duas cumprem** (bug da titular, 24/09: "compartilhei e as pessoas não conseguem ver"):
  1. **O plano** (`todogreen_planner_plans`, `members_json`) só chega a quem consegue **abrir a vertical NESTE espaço** — `resolveTodoGreenAccess` não tem porta para `memberships` (o espaço do app), então quem só foi convidado pelo "Seu Funcionário" é aceito na lista mas recusado na porta (403). `alcancaEspacoNaVertical(env, pessoa, ownerId)` em `todogreen-access.js` é a resposta única a "essa pessoa vai ver?" (reusa `resolveTodoGreenAccess` com `registrarAcesso: false` para não carimbar último acesso): `/planner/pessoas` devolve `alcancaPlanner` por pessoa e POST/PATCH de plano devolvem `membrosSemAcesso`. A lista **é mantida** (liberar em Acessos depois faz o plano aparecer sem recompartilhar); o que muda é a tela avisar em vez de fingir que deu certo. **Abrir** o Planner exige só `read` (`MODULE_IMPLEMENTATION.planner.permission = ["read", "planner:manage"]`, alternativas): o servidor já entregava planos ao auditor, e a casca escondia o item do menu — `planner:manage` continua sendo a régua de **gerir** (servidor + `podeGerir` na tela). A `PlannerPage` manda `?owner=espacoId` (o `ownerId` que `/api/todogreen/access` confirmou) em toda chamada, para ler/gravar no mesmo espaço em que Acessos libera gente.
  2. **As ações (tarefas) do plano moram no `db.tasks` do DONO do espaço da vertical** (`workspaces.user_id = access.ownerId`), e o servidor as lê e grava com o corte de visibilidade do PLANO: quem vê o plano vê TODAS as ações dele (bug da titular, 24/09, segunda rodada: "permanece invisível para os demais"). Antes, a tela gravava no workspace de QUEM estava logado — cada colega via só o que ele mesmo criou, e quem entrou só pela vertical (tenant_users) abria o plano com o quadro vazio. Rotas em `todogreen-planner.js`: `GET /planner/acoes` (ações de todos os planos visíveis; `?plano=` filtra), `GET /planner/planos/:id/acoes`, `PUT /planner/planos/:id/acoes/:taskId` (upsert; exige `planner:manage`; o servidor monta a task com `aplicarEdicaoPlannerNaTarefa`, valida responsável/vínculos comerciais, recusa com 409 um id que já é de outra tarefa fora do plano) e `DELETE` idem. Gravação por `gravarNoEspaco`: revisão condicional com retentativa, o mesmo padrão do `/api/tasks/action` — **não é um segundo store**; `todogreen_planner_tasks` segue só para migração. PATCH de plano e arquivamento também atualizam as ações no quadro (`aplicarPartilhaDoPlanoNaTarefa` / `desvincularTarefaDoPlanner`) e devolvem `espaco: {revision, updatedAt, tarefas, removidas}`. No front, toda escrita passa por `workspaceServerWrite(espacoId, run)` (em `useDatabase`, repassado por `PrimaryAppRouter` → `LogisticsVertical` → `PlannerPage`): se a aba está com o MESMO workspace aberto, salva o pendente antes e adota a revisão/tarefas devolvidas depois — sem isso o próximo salvamento automático da titular bateria em 409 e abriria o aviso de conflito. A `PlannerPage` recarrega as ações ao voltar para a aba e a cada 45 s (visível), aplica a gravação de forma otimista e, quando o workspace aberto no app não é o do espaço, **resgata uma vez** as ações antigas presas no workspace pessoal (PUT no servidor + remoção local). A partilha do plano ainda desce para as ações (`partilhaDoPlanoNaTarefa`: pessoas específicas + quem criou o plano → `sharedWith` com `sharingPermission: "editar"`; todo o espaço → `espaco_todo`; privado desfaz só o que o Planner ligou) — isso serve ao To Do/Meu Dia (`canSeeTask`), não ao Planner, que já filtra pelo plano.
