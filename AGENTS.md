...
- **Compartilhar um plano é uma promessa em DUAS camadas, e a tela só pode prometer o que as duas cumprem** (bug da titular, 24/09: "compartilhei e as pessoas não conseguem ver"):
  ...
  2. **As tarefas** moram em `db.tasks` (workspace do app), e quem as vê é `canSeeTask`. Sem ponte, a pessoa abria o plano e via o quadro vazio. `partilhaDoPlanoNaTarefa(tarefa, plano)` (`plannerIntegrationDomain.js`) desce a partilha do plano para as tarefas dele: pessoas específicas → `sharedWith` + `sharingPermission: "editar"`; todo o espaço → `visibility: "espaco_todo"`; privado → desfaz **só** o que o Planner ligou (marcas `plannerSharedWith`/`plannerVisibility`; o compartilhamento manual fica). Roda dentro de `aplicarEdicaoPlannerNaTarefa`, em `desvincularTarefaDoPlanner` e, ao criar/recompartilhar um plano, sobre `db.tasks` inteiro via `onSyncPlanSharing` (`aplicarPartilhaDoPlanoNaTarefa` só toca tarefas daquele plano e devolve a mesma referência quando não há nada a fazer). Limite conhecido: `mergeRecordsFromMember` tranca `sharedWith`/`visibility` para quem não é dono do registro — a ponte vale para tarefas de quem é dono delas (normalmente a titular), e a pessoa precisa estar no espaço do app (memberships + `sf-space`) para o `db.tasks` dela ser o da titular.
