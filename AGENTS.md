...
`src/App.jsx` ainda é grande (~20,8 mil linhas), mas a **camada de lógica pura** (sem React/JSX) está sendo movida para `src/domain.js`: helpers fundamentais (`uid`, `today`, `contactLinks`), gamificação (`DEFAULT_LEVELS`, `computeUserPoints`, `levelForPoints`, `levelProgress`, `computeAchievements`), `computeMyWork` e `computeBusinessInsights`. **Padrão (lógica pura):** `App.jsx` importa de `./domain.js` para uso interno **e reexporta** o que os testes consomem via `import { ... } from "./App"` — assim testes existentes não quebram. Ao adicionar nova lógica pura/testável, colocar em `domain.js` (não em `App.jsx`) e, se algum teste importar de `./App`, adicionar à linha de reexport.

**Padrão (componentes):** componentes-folha migram para `src/components/X.jsx` com `export default`, e o `App.jsx` os reimporta (`import X from "./components/X.jsx"`) deixando no lugar da definição antiga um comentário `// Movido para ./components/X.jsx.`. Já extraídos assim: `Modal`, `Markdown`, `SharingFields`, `Anexos`, `Toast`, `AppUpdate`. Como os testes de UI renderizam o `App` inteiro (`import App from "./App"`), mover um componente para módulo próprio e reimportá-lo mantém a suíte verde — a superfície do `App` não muda. Comece pelos componentes de menor acoplamento (props → JSX, sem depender de closures do `App`). Próximos passos da quebra (ainda não feitos): extrair componentes de página maiores com `React.lazy` para code-splitting.
