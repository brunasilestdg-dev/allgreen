...
import FirstAccessPassword from "./features/auth/FirstAccessPassword.jsx";
import { exigeTrocaDeSenha } from "./features/auth/authDomain.js";
import { useEntradaPorConvite } from "./features/auth/useEntradaPorConvite.js";
import { useStatusDaSessao } from "./session/useStatusDaSessao.js";
import {
...
function useDatabase() {
  ...
  // sessão: quem decide é o Worker (ver src/session/useStatusDaSessao.js).
  // Enquanto ele não valida o token, a vertical To Do Green não é renderizada.
  const { sessionStatus, markSessionAuthenticated, encerrarSessaoLocal } =
    useStatusDaSessao(userId, setDb);
  ...
  // Sessão expirada no meio do uso: só a identidade sai; a cópia local do
  // espaço continua no navegador para a próxima entrada.
  const logoutFromExpiredSession = encerrarSessaoLocal;
...
// Entrada, verificação, recuperação, convite e primeiro acesso: src/features/auth/.
...
export default function App() {
  ...
  if (exigeTrocaDeSenha({ user: db.user, sessionStatus, routeKind: primaryRoute.kind }))
