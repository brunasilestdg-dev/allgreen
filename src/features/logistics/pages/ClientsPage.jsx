...
export default function ClientsPage({ authHeaders, opportunities = [], contracts = [], operations = [], financial = [], tasks = [], comments = [], onComment, interactions = [], onInteraction, onNavigate, setToast, onCreateTask, onCompletarTarefa, currentUserId, remetenteNome = "", assinaturaEmail = "", espacoId = "", onClientContextChange, onClientesChange }) {
  ...
  // Nunca vazar um id técnico (uuid/hash) onde deveria aparecer o nome da tabela:
  // um código como "8f3a1c…" na tela lê como bug. Código legível passa; id técnico
  // vira um rótulo neutro.
  const rotuloTabela = (valor) => {
    const s = String(valor || "").trim();
    if (!s) return "Não informada";
    if (/^[0-9a-f]{8,}$/i.test(s) || /^[a-z]+_[0-9a-z-]{6,}$/i.test(s)) return "Tabela personalizada";
    return s;
  };
      ...
      <div className={`tdg-crm-detail-grid tdg-account-tab-${detailTab}`}><main>
        ...
        {comercial && <section className="tdg-crm-detail-section tdg-account-panel tdg-account-comercial">
          ...
          <dl className="tdg-crm-account-data tdg-comercial-grid">
            ...
            <div><dt>Tabela negociada</dt><dd>{rotuloTabela(comercial.contrato?.tabelaPrecoNome || comercial.contrato?.tabelaPrecoId || comercial.contrato?.cenarioId)}</dd></div>
