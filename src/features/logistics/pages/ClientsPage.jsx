...
export default function ClientsPage({ authHeaders, opportunities = [], contracts = [], operations = [], financial = [], comments = [], onComment, interactions = [], onInteraction, onNavigate, setToast, onCreateTask, currentUserId, espacoId = "", onClientContextChange }) {
  ...
  return <section className="tdg-panel tdg-page tdg-clients-page">
    ...
    {!selected && <>
      ...
      {showCreate && <Modal title="Nova conta" onClose={() => setShowCreate(false)}><form className="tdg-client-admin-form tdg-form-em-modal" onSubmit={createClient}><div className="tdg-form-row"><label><span>Nome</span><input required autoFocus value={clientForm.nome} onChange={(e) => setClientForm({ ...clientForm, nome: e.target.value })} /></label><label><span>Documento</span><input value={clientForm.documento} onChange={(e) => setClientForm({ ...clientForm, documento: e.target.value })} /></label><label><span>Segmento</span><input value={clientForm.segmento} onChange={(e) => setClientForm({ ...clientForm, segmento: e.target.value })} /></label><label><span>Classificação</span><select value={clientForm.tier} onChange={(e) => setClientForm({ ...clientForm, tier: e.target.value })}>{TODO_GREEN_ACCOUNT_TIERS.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>Momento</span><select value={clientForm.stage} onChange={(e) => setClientForm({ ...clientForm, stage: e.target.value })}>{TODO_GREEN_ACCOUNT_STAGES.map((item) => <option key={item}>{item}</option>)}</select></label></div><div className="tdg-form-actions"><button type="button" onClick={() => setShowCreate(false)}>Cancelar</button><button className="tdg-action"><Plus size={16} />Cadastrar conta</button></div></form></Modal>}
