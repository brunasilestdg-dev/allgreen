...
export const FORMS = {
  ...
  warehouses:{ initial:{codigo:"",nome:"",tipo:"proprio",cep:"",endereco:""}, fields:[["codigo","Código","text",true],["nome","Nome","text",true],["tipo","Tipo","select",false,"warehouseKind"],["cep","CEP (preenche o endereço)","cep",false,"endereco"],["endereco","Endereço"]] },
  ...
  operationalUnits:{ initial:{code:"",name:"",kind:"base",document:"",cep:"",addressText:"",status:"draft"}, fields:[["code","Código"],["name","Nome da base / unidade","text",true],["kind","Tipo","select",false,"unitKind"],["document","CNPJ, se houver"],["cep","CEP (preenche o endereço)","cep",false,"addressText"],["addressText","Endereço"],["status","Status","select",false,"genericStatus"]] },
...
export const payloadFor = (tab,form) => {
  ...
  if(["operationalUnits","companyProfiles"].includes(tab)){body.address=form.addressText?{full:form.addressText,...(form.cep?{cep:form.cep}:{})}:{};delete body.addressText;}
  delete body.cep; // auxiliar de tela: já preencheu o endereço (e entrou no address quando estruturado)
