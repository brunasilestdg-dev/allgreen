...
import { SearchableSelect } from "../../../design-system/index.js";
import { parseBrNumber } from "../../../domain.js";
import { comRotulo } from "../rotulosDomain.js";

// Opções para o combobox com busca, a partir das coleções da vertical.
const opcoesClientes = (clients) => clients.map((item) => ({ value: item.id, label: item.name || item.nome || item.id }));
const opcoesContratos = (contracts) => contracts.map((item) => ({ value: item.id, label: item.titulo || item.title || item.id }));
const opcoesOperacoes = (operations) => operations.map((item) => ({ value: item.id, label: item.referencia || item.reference || item.id }));
...
function ServiceOrders({ authHeaders, clients, contracts, operations, setToast }) {
  ...
  const create = async (event) => {
    ...
    try {
      await request("service-orders", authHeaders, { method: "POST", body: JSON.stringify({ ...form, quantity: parseBrNumber(form.quantity), unitPrice: parseBrNumber(form.unitPrice), discountAmount: parseBrNumber(form.discountAmount), taxAmount: parseBrNumber(form.taxAmount) }) });
  ...
  return <section className="tdg-panel tdg-txn-page"><div className="tdg-section-head"><div><span className="tdg-kicker">PLANEJAMENTO E PRODUTOS</span><h2>Aceite e ordens de serviço</h2><p>Planejamento/Produtos aceita a viagem e libera a OS. Operação executa depois; Financeiro entra com CT-e, documento fiscal, título e baixa.</p></div><div className="tdg-page-actions"><strong>{records.length} ordem(ns)</strong><button type="button" className="tdg-action" onClick={() => setNovaAberta(true)}><Plus size={16} />Nova OS</button></div></div>
    ...
    <form className="tdg-txn-form tdg-form-em-modal" onSubmit={create}>
      <SearchableSelect label="Cliente" required placeholder="Buscar cliente…" options={opcoesClientes(clients)} value={form.clientId} onChange={(val) => setForm((v) => ({ ...v, clientId: val || "", contractId: "" }))} />
      <SearchableSelect label="Contrato aprovado e assinado" required placeholder="Buscar contrato…" options={opcoesContratos(eligibleContracts)} value={form.contractId} onChange={(val) => setForm((v) => ({ ...v, contractId: val || "" }))} hint={form.clientId && !eligibleContracts.length ? "Nenhum contrato elegível para este cliente." : undefined} />
      <SearchableSelect label="Operação vinculada" placeholder="Vincular depois" options={opcoesOperacoes(operations.filter((item) => !form.clientId || (item.clientId || item.clienteId) === form.clientId))} value={form.operationId} onChange={(val) => setForm((v) => ({ ...v, operationId: val || "" }))} />
      ...
      <label><span>Preço unitário</span><input type="text" inputMode="decimal" value={form.unitPrice} onChange={(e) => setForm((v) => ({ ...v, unitPrice: e.target.value }))} placeholder="Herda do contrato/simulação" /><small>Deixe em branco para herdar o valor negociado do contrato (ou o preço da simulação).</small></label>
...
function Ciot({ authHeaders, clients, contracts, operations, setToast }) {
  ...
  const save = async (event) => {
    event.preventDefault(); setSaving(true);
    try {
      const body = { ...form, fields: { rntrc: form.rntrc }, environment: integrationForm.environment, certificateType: integrationForm.certificateType, operationId: selectedOrder?.operationId || "", freightAmount: parseBrNumber(form.freightAmount || selectedOrder?.netAmount || 0), floorAmount: parseBrNumber(form.floorAmount || 0) };
  ...
  return <section className="tdg-panel tdg-txn-page"><div className="tdg-section-head"><div><span className="tdg-kicker">ANTT · API DIRETA · CIOT</span><h2>Geração e controle de CIOT</h2><p>Integração direta para ETC/frota própria sem IPEF: certificado ICP-Brasil A1/A3, payload regulatório, piso mínimo e retorno do código governamental de 12 dígitos.</p></div><div className="tdg-page-actions"><strong>{records.length} CIOT(s)</strong><button type="button" className="tdg-action" onClick={() => setEmissaoAberta(true)}><ReceiptText size={16} />Emitir CIOT</button></div></div>
    ...
    <form className="tdg-txn-form tdg-form-em-modal" onSubmit={save}>
      <SearchableSelect label="OS vinculada" placeholder="Sem OS" options={orders.map((item) => ({ value: item.id, label: `${item.number} · ${clientName(clients, item.clientId)}` }))} value={form.serviceOrderId} onChange={(val) => { const order = orders.find((item) => item.id === val); setForm((v) => ({ ...v, serviceOrderId: val || "", freightAmount: order?.netAmount ? String(order.netAmount) : v.freightAmount, startsAt: order?.scheduledStartAt || v.startsAt, endsAt: order?.scheduledEndAt || v.endsAt })); }} hint={selectedContract ? `Contrato: ${selectedContract.titulo || selectedContract.title || selectedContract.id}` : undefined} />
      ...
      <label><span>Frete declarado</span><input required type="text" inputMode="decimal" value={form.freightAmount} onChange={(e) => change("freightAmount", e.target.value)} /></label>
      <label><span>Piso mínimo calculado</span><input type="text" inputMode="decimal" value={form.floorAmount} onChange={(e) => change("floorAmount", e.target.value)} /></label>
...
function Titles({ authHeaders, clients, setToast }) {
  ...
  const settle = async (event) => { event.preventDefault(); try { await request(`titles/${paying.id}/settle`, authHeaders, { method: "POST", body: JSON.stringify({ amount: parseBrNumber(amount), settledAt: new Date().toISOString(), method: "pix" }) }); setToast?.("Baixa registrada"); setPaying(null); await load(); } catch (error) { setToast?.(error.message); } };
  const criar = async (event) => { event.preventDefault(); setSalvando(true); try { await request("titles", authHeaders, { method: "POST", body: JSON.stringify({ ...novo, amount: parseBrNumber(novo.amount) }) }); setToast?.("Título lançado"); setNovoAberto(false); setNovo(TITULO_VAZIO); await load(); } catch (error) { setToast?.(error.message); } finally { setSalvando(false); } };
  ...
  return <section className="tdg-panel tdg-txn-page"><div className="tdg-section-head"><div><span className="tdg-kicker">RECEBÍVEIS E CONCILIAÇÃO</span><h2>Títulos e baixas</h2><p>Competência, vencimento, parcela, origem fiscal/documental e saldo real em um único razão.</p></div><div className="tdg-section-head-acoes"><strong>{BRL.format(openTotal)} em aberto</strong><button type="button" className="tdg-action" onClick={() => { setNovo(TITULO_VAZIO); setNovoAberto(true); }}><Plus size={16} />Novo título</button></div></div>
    {novoAberto && <Modal title="Novo título financeiro" onClose={() => setNovoAberto(false)}>
      <form className="tdg-txn-close tdg-form-em-modal" onSubmit={criar}>
        ...
        {novo.kind === "receivable"
          ? <SearchableSelect label="Cliente" placeholder="Buscar cliente… (opcional)" options={opcoesClientes(clients)} value={novo.clientId} onChange={(val) => mudar("clientId", val || "")} />
        ...
        <label><span>Valor</span><input required type="text" inputMode="decimal" value={novo.amount} onChange={(e) => mudar("amount", e.target.value)} placeholder="0,00" /></label>
    ...
    {paying && <Modal title={`Baixa de ${paying.number}`} onClose={() => setPaying(null)}>
      <form className="tdg-txn-close tdg-form-em-modal" onSubmit={settle}><div><small>Saldo atual {BRL.format(paying.open_amount)}</small></div><label><span>Valor</span><input required type="text" inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} /></label><div className="tdg-form-actions"><button type="button" onClick={() => setPaying(null)}>Cancelar</button><button className="tdg-action"><CircleDollarSign size={17} />Confirmar baixa</button></div></form>
    </Modal>}
  </section>;
}
...
function Costs({ authHeaders, clients, contracts, operations, setToast }) {
  ...
  const allocated = form.allocations.reduce((sum, item) => sum + parseBrNumber(item.amount), 0); const remaining = parseBrNumber(form.amount) - allocated;
  ...
  const save = async (event) => { event.preventDefault(); setSaving(true); try { await request("costs", authHeaders, { method: "POST", body: JSON.stringify({ ...form, amount: parseBrNumber(form.amount), allocations: form.allocations.map((item) => ({ ...item, amount: parseBrNumber(item.amount) })) }) }); setToast?.("Custo e rateios registrados"); setForm({ description: "", amount: "", competenceDate: today(), supplierId: "", documentNumber: "", allocations: [allocationEmpty()] }); setNovoAberto(false); await load(); } catch (error) { setToast?.(error.message); } finally { setSaving(false); } };
  return <section className="tdg-panel tdg-txn-page"><div className="tdg-section-head"><div><span className="tdg-kicker">CUSTO MULTIDIMENSIONAL</span><h2>Rateio de custos</h2><p>Um custo pode ser distribuído entre OS, operação, cliente, contrato, veículo, fornecedor e centro de custo.</p></div><div className="tdg-page-actions"><strong>{records.length} custo(s)</strong><button type="button" className="tdg-action" onClick={() => setNovoAberto(true)}><Plus size={16} />Novo custo</button></div></div>
    ...
    <form className="tdg-txn-cost tdg-form-em-modal" onSubmit={save}><div className="tdg-txn-form"><label><span>Descrição</span><input required value={form.description} onChange={(e) => setForm((v) => ({ ...v, description: e.target.value }))} /></label><label><span>Valor total</span><input required type="text" inputMode="decimal" value={form.amount} onChange={(e) => setForm((v) => ({ ...v, amount: e.target.value }))} /></label><label><span>Competência</span><input required type="date" value={form.competenceDate} onChange={(e) => setForm((v) => ({ ...v, competenceDate: e.target.value }))} /></label><label><span>Documento</span><input value={form.documentNumber} onChange={(e) => setForm((v) => ({ ...v, documentNumber: e.target.value }))} /></label></div>
      ...
      {form.allocations.map((item, index) => <div className="tdg-txn-allocation" key={index}><label><span>Valor</span><input required type="text" inputMode="decimal" value={item.amount} onChange={(e) => changeAllocation(index, "amount", e.target.value)} /></label><SearchableSelect label="Cliente" placeholder="Sem vínculo" options={opcoesClientes(clients)} value={item.clientId} onChange={(val) => changeAllocation(index, "clientId", val || "")} /><SearchableSelect label="Contrato" placeholder="Sem vínculo" options={opcoesContratos(contracts)} value={item.contractId} onChange={(val) => changeAllocation(index, "contractId", val || "")} /><SearchableSelect label="Operação" placeholder="Sem vínculo" options={opcoesOperacoes(operations)} value={item.operationId} onChange={(val) => changeAllocation(index, "operationId", val || "")} /><label><span>OS</span><input value={item.serviceOrderId} onChange={(e) => changeAllocation(index, "serviceOrderId", e.target.value)} placeholder="ID da OS" /></label><label><span>Veículo</span><input value={item.vehicleId} onChange={(e) => changeAllocation(index, "vehicleId", e.target.value)} /></label><label><span>Fornecedor</span><input value={item.supplierId} onChange={(e) => changeAllocation(index, "supplierId", e.target.value)} /></label><label><span>Centro de custo</span><input value={item.costCenterId} onChange={(e) => changeAllocation(index, "costCenterId", e.target.value)} /></label>{form.allocations.length > 1 && <button type="button" onClick={() => setForm((v) => ({ ...v, allocations: v.allocations.filter((_, position) => position !== index) }))}>Remover</button>}</div>)}
