...
export default function PlannerPage({ authHeaders, setToast, currentUserId, role, espacoId = "" }) {
  ...
  const [formPlano, setFormPlano] = useState({ name: "", visibility: "private", description: "" });
  ...
  const criarPlano = async (evento) => {
    ...
    setOcupado("salvando");
    try {
      const novo = await request("/planos", authHeaders, { method: "POST", body: JSON.stringify(formPlano) });
      setModalPlano(false);
      setFormPlano({ name: "", visibility: "private", description: "" });
              ...
              {ocupado === "carregando" && planos.length === 0 ? (
                ...
                planos.map((p) => (
                  ...
                  >
                    ...
                    {p.name}
                    {p.visibility === "private" ? <Lock size={12} /> : <Users size={12} />}
                  </button>
                ))
              )}
            </div>
        ...
        <Modal title="Novo plano" onClose={() => setModalPlano(false)}>
          <form className="tdg-planner-form" onSubmit={criarPlano}>
            ...
            <fieldset className="tdg-planner-visibility">
              ...
              {PLANNER_VISIBILITIES.map((v) => (
                <label key={v.id} className="tdg-planner-radio">
                  <input type="radio" name="visibility" checked={formPlano.visibility === v.id} onChange={() => setFormPlano((f) => ({ ...f, visibility: v.id }))} />
                  <span>{v.id === "private" ? <Lock size={14} /> : <Users size={14} />} <strong>{v.label}</strong> — {v.ajuda}</span>
                </label>
              ))}
            </fieldset>
