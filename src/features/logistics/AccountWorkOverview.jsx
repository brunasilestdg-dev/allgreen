...
import { accountWorkTasks, taskWorkRoute, estruturarNotasDaConta } from "./accountWorkDomain.js";
...
export default function AccountWorkOverview({ client, contacts = [], interactions = [], tasks = [], onTab, onNavigate }) {
  ...
  const notas = client.notes ? estruturarNotasDaConta(client.notes) : null;
  return <section className="tdg-account-work tdg-account-panel tdg-account-summary tdg-account-next" aria-label="Pessoas, conversas e próximos passos">
    ...
    {notas && (notas.linhas.length > 0 || notas.updates.length > 0) && <div className="tdg-account-clue">
      <strong>O que já sabemos</strong>
      {notas.linhas.length > 0 && <ul className="tdg-account-clue-campos">
        {notas.linhas.map((linha, i) => <li key={i}>{linha}</li>)}
      </ul>}
      {notas.updates.length > 0 && <div className="tdg-account-clue-updates">
        <span>Últimos updates</span>
        <ul>{notas.updates.map((u, i) => <li key={i}>{u}</li>)}</ul>
      </div>}
    </div>}
