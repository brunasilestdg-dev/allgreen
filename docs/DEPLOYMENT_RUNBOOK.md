...
| 2026-09-24 23:30 | `f856e9f6fadf` (PR #17) | GitHub Actions — botão "Publicar" (`deploy.yml`, `workflow_dispatch`, execução 2) | `888e430f-…` | não conferido: às 00:14 a produção já estava no `sha` seguinte |
| 2026-09-24 23:53 | `f6b9548f6c84` (= `main`, PR #21) | GitHub Actions — botão "Publicar" (execução 3) | `4e69e63d-…` | `/api/system/version` às 00:14 UTC de 25/09: `publishedBy: github-actions`, `branch: main`, `migrations.expected` 148, última `0145_busca_vetores`; `/api/status` com `antiRobo` e `aiGateway` desligados |
| 2026-09-25 01:40 | `92dfbc432c78` (= `main`, PR #15) | GitHub Actions — botão "Publicar" (execução 4), disparado depois de 6 min sem o Workers Builds publicar o merge | `97a1c731-…` | `/api/system/version` `92dfbc432c78`, `publishedBy: github-actions`, `migrations.expected` 148, última `0145_busca_vetores`; `/api/status` sem o bloco `roadmap`; `/f/inexistente-smoke` → 404 do Worker (antes 200 com a SPA); `/api/rota-que-nao-existe` → 404 JSON (antes 200 com a SPA); `/api/todogreen/records` e `/api/todogreen/treasury` → 401 sem sessão; SPA `/` e `/todogreen` 200; gate no próprio fluxo: verify, build e e2e crítico |
| 2026-09-25 02:07 | `8744fb58342d` (= `main`: PRs #25, #23, #24 — L8 — e #10) | GitHub Actions — botão "Publicar" (execução 5), depois de 6 min sem o Workers Builds publicar | `1cdefbbf-…` | `/api/system/version` `8744fb58342d`, `publishedBy: github-actions`, `migrations.expected` 148; gate no próprio fluxo |
| 2026-09-25 02:24 | `44f3445533b0` (= `main`, PR #22 — D1) | `npm run deploy:cloudflare` direto (migrations + `wrangler deploy`), com `CLOUDFLARE_API_TOKEN` só no ambiente do comando — sem GitHub Actions, a pedido da responsável; gate local antes, na mesma `main`: verify (lint 0 erros, unit 5.004, worker 1.419), build e e2e crítico 11/11 | `44637378-…` | `/api/system/version` `44f3445533b0`, `publishedBy: manual`, `branch: main`, `migrations.expected` 148 ("No migrations to apply"); `/api/status` operacional; `/f/inexistente-smoke` 404 do Worker; `/api/rota-que-nao-existe` 404 JSON; `/api/todogreen/records` e `/api/todogreen/transactions/titles` 401 sem sessão; `/` e `/todogreen` 200 |
| 2026-09-25 12:33 | `bab4fa0cd79b` (ramo local `codex/design-system-refresh`, commit fora do GitHub) | manual, fora de CI (`publishedBy: manual`), pela titular com a Codex | — | `/api/system/version` conferido às 12:36 e 12:42 UTC com esse `sha`; `sw.js` `allgreen-v288`; CSS index-Bda9-ykS.css, com a folha AllGreenDesignSystemFinal ativada. A `main` estava em `c2cb64c` (merge do PR #27 às 12:35), que não chegou ao ar: foi integrado a esta versão no PR #28 |
| 2026-09-25 15:59 | `8f103af05e76` (= `main`, PR #28 — integra `bab4fa0` ao design system do PR #27) | GitHub Actions — botão "Publicar" (execução 6, 15:47–16:00), depois de 6 min sem o Workers Builds publicar o merge | `8a7cc30b-…` | `/api/system/version` `8f103af05e76`, `publishedBy: github-actions`, `branch: main`, `migrations.expected` 148, última `0145_busca_vetores`; `/api/status` e banco operacionais; `/api/todogreen/records`, `/api/todogreen/treasury` e `/api/todogreen/transactions/titles` → 401 sem sessão; `/`, `/todogreen`, `/portal-tms`, `/portal-cliente` e `/portal-motorista` 200; `/f/inexistente-smoke` 404 do Worker; `/api/rota-que-nao-existe` 404 JSON; `sw.js` `allgreen-v289`; CSS servido index-CQ7iHIK3.css, o mesmo do build validado; entrada da vertical sem erro de JavaScript no Chromium, no desktop e no celular; gate no próprio fluxo: verify, build e e2e crítico |
