...
| Gate local do estado final da `main` (com as correções acima) | lint 0 erros; unit **357/357** (o teste do To Do voltou a passar com a correção do Codex; guarda de tokens corrigida); worker **107/107** (jornada transversal corrigida); build ok; E2E crítico **11/11**; visual **4/4** (baselines regenerados e revalidados) | PR #379 |
| **Produção voltou a publicar** | merge do #379 às 18:06 UTC → `/api/system/version` = `3d83b2b7e6fe` às 18:07:30 (70 s), depois de 3h27 presa em `037ab25` | causa confirmada: o E2E dentro do `deploy:cloudflare`; o deploy command precisa continuar sem navegador |
